//! SciRS2 v0.2.0 Comprehensive Performance Benchmark Suite
//!
//! This is the main orchestrator for the v0.2.0 performance validation suite.
//! It runs comprehensive benchmarks across all SciRS2 modules and generates
//! performance baselines for the v0.2.0 release.
//!
...
//! - SIMD operations: 5-15x speedup vs naive implementations
//! - Matrix operations: competitive with BLAS (via OxiBLAS)
//! - FFT: competitive with rustfft/OxiFFT
//! - Parallel operations: near-linear scaling up to 8 cores
...
fn save_results() {
    unsafe {
        if let Some(ref results) = BENCHMARK_RESULTS {
            ...
            let mut file = File::create("/tmp/scirs2_v020_benchmark_results.json")
                .expect("Failed to create results file");
            file.write_all(json.as_bytes())
                .expect("Failed to write results");
            println!(
                "\n✓ Saved {} benchmark results to /tmp/scirs2_v020_benchmark_results.json",
                results.len()
            );
        }
    }
...
fn bench_integration_operations(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("integration_operations");
    group.sampling_mode(SamplingMode::Flat);
    group.measurement_time(Duration::from_secs(5));
    ...
    // Adaptive quadrature
    group.bench_function("quad_adaptive", |bencher| {
        bencher.iter(|| {
...
fn bench_signal_processing(c: &mut Criterion) {
    ...
    let signal_sizes = [100, 500, 1000, 5000];
    let kernel_size = 21;
...
fn benchmark_startup(_: &mut Criterion) {
    init_results();
    println!("\n╔════════════════════════════════════════════════════════════╗");
    println!("║  SciRS2 v0.2.0 Comprehensive Performance Benchmark Suite  ║");
    println!("╚════════════════════════════════════════════════════════════╝\n");
}
