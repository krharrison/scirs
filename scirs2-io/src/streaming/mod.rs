...
///
/// Stateless and stateful transformations for processing large JSONL datasets
/// without loading everything into memory:
/// - `StreamingFilter`: keep only matching records
/// - `StreamingProject`: keep only specified fields
/// - `StreamingRename`: rename fields
/// - `StreamingDeduplicate`: remove duplicates by key (stateful)
/// - `StreamingSort`: external merge-sort
/// - `StreamingGroupBy`: sort-based group-by with pluggable aggregates
/// - `StreamingPipeline`: chain multiple operators
pub mod relational;
