...
impl PersistenceDiagram {
    ...
    pub fn betti_numbers_at(&self, filtration_value: f64) -> Vec<usize> {
        ...
        for p in &self.points {
            if p.birth <= filtration_value && (p.death > filtration_value || p.death.is_infinite())
            {
        ...
        for dim in 1..=(max_dim + 1) {
            ...
            for simplex in &prev_dim_simplices {
                ...
                for v in (last_vertex + 1)..n {
                    ...
                    let max_dist_to_v =
                        simplex.iter().map(|&u| dist[[u, v]]).fold(0.0f64, f64::max);
...
impl PersistenceImage {
    ...
    pub fn transform(&self, diagram: &PersistenceDiagram) -> Result<Array2<f64>> {
        ...
        let b_max = self
            .birth_range
            .1
            .max(pts.iter().map(|(b, _)| *b).fold(0.0_f64, f64::max));
        let p_min = self.persistence_range.0;
        let p_max = self
            .persistence_range
            .1
            .max(pts.iter().map(|(_, p)| *p).fold(0.0_f64, f64::max));
...
mod tests {
    ...
    fn test_bottleneck_distance_different_diagrams() {
        ...
        assert!(
            dist > 0.0,
            "Different diagrams should have positive distance"
        );
    }
    ...
    fn test_persistence_landscape() {
        ...
        let landscape =
            PersistenceLandscape::compute(&diagram, 2, 20, 0).expect("landscape compute");
