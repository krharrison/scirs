    ...
    pub fn topological_order(&self) -> Result<Vec<usize>> {
        ...
        while let Some(id) = queue.pop_front() {
            ...
            let node = self.nodes.get(id).ok_or_else(|| {
                TimeSeriesError::InvalidInput(format!("Node id {id} not found"))
            })?;
...
pub fn middle_out_forecast(
    ...
    if middle_to_bottom_proportions.shape()[0] != k
        || middle_to_bottom_proportions.shape()[1] != m
    {
        return Err(TimeSeriesError::InvalidInput(format!(
    ...
    for &id in &topo_order {
        let node = hierarchy.node(id).ok_or_else(|| {
            TimeSeriesError::InvalidInput(format!("Node id {id} not found"))
        })?;
        if node.is_leaf() {
            continue;
        ...
        let children_all_middle = node
            .children
            .iter()
            .all(|c| middle_node_ids.contains(c));
