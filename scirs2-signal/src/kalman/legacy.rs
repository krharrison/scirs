...
pub fn kalman_filter(
    ...
    let mut x_state = match initial_x {
        Some(x0) => {
            if x0.len() != n_states {
                return Err(SignalError::DimensionMismatch(
    ...
    for i in 0..n_samples {
        ...
        x_state = &x_pred + &k.dot(&innovation);
        p = &p_pred - &k.dot(&innovation_cov).dot(&k.t());

        // Store state
        for j in 0..n_states {
            x_history[[i, j]] = x_state[j];
        }
    ...
    HJ: Fn(&Array1<f64>) -> Array2<f64>,
{
    ...
    for i in 0..n_samples {
        // Predict step
        let x_pred = f_func(&x_state);
        let f_jac = f_jacobian(&x_state);
    ...
    H: Fn(&Array1<f64>) -> Array1<f64>,
{
    ...
    for i in 0..n_samples {
        ...
        let sigma_points = generate_sigma_points(&x_state, &p, gamma)?;
    ...
    H: Fn(&Array1<f64>) -> Array1<f64>,
{
    ...
    for _ in 0..n_ensemble {
        ...
        for j in 0..n_states {
            perturbation[j] = rng.sample(scirs2_core::random::StandardNormal);
        }
