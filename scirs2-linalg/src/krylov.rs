...
fn mv<F>(a: &Array2<F>, x: &Array1<F>) -> Array1<F>
where
...
#[inline]
fn dot_vec<F: Float + NumAssign + Sum>(a: &Array1<F>, b: &Array1<F>) -> F {
    a.iter()
        .zip(b.iter())
        .map(|(&x, &y)| x * y)
...
fn diagonal_condition_estimate<F: Float + NumAssign + Sum + ScalarOperand>(
    a: &Array2<F>,
) -> F {
    let n = a.nrows();
